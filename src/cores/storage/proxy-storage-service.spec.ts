...
describe('ProxyStorageService', () => {
    ...
    it('should pass basic service', async () => {
        ...
        expect2(service.asKey$('test', '1:23:45'), '_id').toEqual({ _id: 'TT:test:1-23-45' });
        expect2(service.asKey$('test', '1:23:45')).toEqual({
            ns: 'TT',
            type: 'test',
            id: '1:23:45',
            _id: 'TT:test:1-23-45',
        ...
        expect2(storage.asTime()).toEqual({
            createdAt: current + 10,
            updatedAt: current + 100,
            deletedAt: current + 1000,
        });
        expect2(storage.asTime(current)).toEqual({
            createdAt: current + 10,
            updatedAt: current + 100,
            deletedAt: current + 1000,
        });
    ...
    const build_test_scenario_by_type = (type: 'dummy' | 'dynamo') => async () => {
        ...
        if (1) {
            ...
            expect2(storage.asTime()).toEqual({
                createdAt: 11223344 + 10,
                updatedAt: 11223344 + 100,
                deletedAt: 11223344 + 1000,
            });
            //* restore origin.
        ...
        expect2(await storage.doRead('sequence' as MyType, 'test').catch(GETERR)).toEqual(
        ...
        expect2((await storage.nextUuid()).split('-').length).toEqual(
            'd01764cd-9ef2-41e2-9e88-68e79555c979'.split('-').length,
        );
        ...
        expect2(await storage.doUpdate('test', 'bbb', { stereo: 'b' })).toEqual({
            _id: 'TT:test:bbb',
            stereo: 'b',
            updatedAt,
        });
        ...
        if (1) {
            ...
            expect2(await $test.update('bbb', { price: 1000 }), '!updatedAt').toEqual({
                _id: 'TT:test:bbb',
                price: 1000,
                count: 1,
            });
            expect2(await $test.read('bbb'), '!updatedAt').toEqual({
                _id: 'TT:test:bbb',
                stereo: 'b',
                price: 1000,
                count: 1,
        ...
        if (1) {
            ...
            expect2(await $test.readOrCreate(id, { name: 'bob' }).catch(GETERR)).toEqual({
                _id,
                id,
                name: 'bob',
                ns: 'TT',
            ...
            expect2(await $test.delete(id)).toEqual({
                _id,
                id,
                name: 'bob',
                ns: 'TT',
        ...
        if (1) {
            ...
            //* test guard()
            expect2(await $test.guard(id, func(3), 0, 10).catch(GETERR)).toEqual(
                '400 TIMEOUT - model[TT:test:a01].lock = 5',
            ); // +0 cycle waiting.
            expect2(await $test.read(id).catch(GETERR)).toEqual({ ...expected, lock: 5 });
        ...
        if (1) {
            ...
            expect2(await $test.save(id, { name: 'bob' }).catch(GETERR)).toEqual({
                _id,
                id,
                ns: 'TT',
                type: 'test',
            ...
            expect2(await $test.read(id).catch(GETERR), 'id,name,createdAt,updatedAt').toEqual({
                id,
                name: 'bob',
                createdAt,
                updatedAt: createdAt,
            ...
            // meta save.
            expect2(await $test.save(id, { tick: 0 } as any).catch(GETERR)).toEqual({
                _id,
                meta: { tick: 0 },
                updatedAt,
            });
            expect2(await $test.read(id).catch(GETERR), 'id,name,meta,createdAt,updatedAt').toEqual({
                id,
                name: 'guk',
                meta: { tick: 0 },
                createdAt,
            ...
            //* internal fields
            // constant member
            expect2(await $test.save(id, { COUNT: '1' } as any).catch(GETERR)).toEqual({
                _id,
                meta: { COUNT: '1' },
                updatedAt,
            });
            ...
            expect2(await $test.read(id).catch(GETERR), 'id,name,createdAt,updatedAt,deletedAt').toEqual({
                id,
                name: 'guk',
                createdAt,
                updatedAt,
            ...
            //* internal object..
            expect2(await storage.save(_id, { name: { a: 1 } } as any).catch(GETERR)).toEqual({
                _id: 'TT:test:bbb',
                name: { a: 1 },
            });
    ...
    //* test ModelUtil.
    it('should pass ModelUtil functions', async () => {
        //* test pop();
    ...
    it('should pass UniqueFieldManager', async () => {
        //* make instance..
        const prepare = async (type: 'dummy' = 'dummy') => {
            const { service, storage: $storage } = instance(
                type == 'dummy' ? 'dummy-account-data.yml' : 'TestCoreTable',
            );
            ...
            expect2($unique.hello()).toEqual(
                'unique-field-manager:test/name:typed-storage-service:test/proxy-storage-service:dummy-storage-service:dummy-account-data/_id',
            );

            //* make 'aaa', no lookup
            expect2(await storage.save('aaa', { name: 'AAA' }), '!_id,!createdAt,!updatedAt').toEqual({
                ns: 'TT',
                type: 'test',
                id: 'aaa',
                name: 'AAA',
            ...
            expect2(await storage.save('bbb', { name: 'BBB' }), '!_id,!createdAt,!updatedAt').toEqual({
                ns: 'TT',
                type: 'test',
                id: 'bbb',
                name: 'BBB',
            ...
            expect2(await storage.read('aaa'), '!_id,!createdAt,!updatedAt').toEqual({
                ns: 'TT',
                type: 'test',
                id: 'aaa',
                name: 'AAA',
            ...
            expect2(await storage.read('bbb'), '!_id,!createdAt,!updatedAt').toEqual({
                ns: 'TT',
                type: 'test',
                id: 'bbb',
                name: 'BBB',
        ...
        if (1) {
            ...
            expect2(await $unique.findOrCreate('AAA', {}).catch(GETERR), 'id,type,name').toEqual({
                id: '1000001',
                type: 'test',
                name: 'AAA',
            });
            ...
            expect2(await storage.delete('1000001', true).catch(GETERR), 'id,type,name').toEqual({
                id: '1000001',
                type: 'test',
                name: 'AAA',
            });
            expect2(await storage.delete('1000001', true).catch(GETERR), 'id,type,name').toEqual(
                '404 NOT FOUND - _id:TT:test:1000001',
            );
            ...
            expect2(
                await $unique.findOrCreate('BBB', { id: 'XYZ', name: 'AAA' }).catch(GETERR),
                'id,type,name',
            ).toEqual('@name (BBB) is not same as (AAA)!');
            expect2(
                await $unique.findOrCreate('BBB', { id: 'XYZ', name: 'BBB' }).catch(GETERR),
                'id,type,name',
            ).toEqual({ id: 'XYZ', type: 'test', name: 'BBB' });
            expect2(
                await $unique.findOrCreate('BBB', { id: 'XYZ', name: 'BBB' }).catch(GETERR),
                'id,type,name',
            ).toEqual({ id: 'XYZ', type: 'test', name: 'BBB' });
            expect2(await storage.delete('XYZ', true).catch(GETERR), 'id,type,name').toEqual({
                id: 'XYZ',
                type: 'test',
                name: 'BBB',
            });
            expect2(await storage.delete('XYZ', true).catch(GETERR), 'id,type,name').toEqual(
                '404 NOT FOUND - _id:TT:test:XYZ',
            );
            expect2(
                await $unique.findOrCreate('BBB', { id: 'XYZ', name: 'BBB' }).catch(GETERR),
                'id,type,name',
            ...
            expect2(await $unique.findOrCreate('CCC', { id: '', name: 'AAA' }).catch(GETERR), 'id,type,name').toEqual(
                '@name (CCC) is not same as (AAA)!',
            );
            expect2(await $unique.findOrCreate('CCC', { id: '', name: 'CCC' }).catch(GETERR), 'id,type,name').toEqual({
                id: '1000002',
                type: 'test',
                name: 'CCC',
            });
            ...
            expect2(await storage.delete('1000002', true).catch(GETERR), 'id,type,name').toEqual({
                id: '1000002',
                type: 'test',
                name: 'CCC',
            });
            expect2(await storage.delete('1000002', true).catch(GETERR), 'id,type,name').toEqual(
                '404 NOT FOUND - _id:TT:test:1000002',
            );
            expect2(await $unique.findOrCreate('CCC', { id: '', name: 'CCC' }).catch(GETERR), 'id,type,name').toEqual({
                id: '1000002',
                type: 'test',
            ...
            expect2(await $unique.findOrCreate('DDD', { name: 'AAA' }).catch(GETERR), 'id,type,name').toEqual(
                '@name (DDD) is not same as (AAA)!',
            );
            expect2(await $unique.findOrCreate('DDD', { name: 'DDD' }).catch(GETERR), 'id,type,name').toEqual({
                id: '1000003',
                type: 'test',
                name: 'DDD',
            });
            ...
            expect2(await storage.delete('1000003', true).catch(GETERR), 'id,type,name').toEqual({
                id: '1000003',
                type: 'test',
                name: 'DDD',
            });
            expect2(await storage.delete('1000003', true).catch(GETERR), 'id,type,name').toEqual(
                '404 NOT FOUND - _id:TT:test:1000003',
            );
            expect2(await $unique.findOrCreate('DDD', { name: 'DDD' }).catch(GETERR), 'id,type,name').toEqual({
                id: '1000003',
                type: 'test',
        ...
        if (1) {
            ...
            expect2(await storage.read($unique.asLookupId('AAA')).catch(GETERR), 'id,type,stereo,meta').toEqual(
                '404 NOT FOUND - _id:TT:test:#name/AAA',
            );
            expect2(await $unique.updateLookup(aaa).catch(GETERR), 'id,type,name').toEqual({
                id: 'aaa',
                type: 'test',
                name: 'AAA',
            });
            ...
            expect2(await $unique.updateLookup({ ...aaa }, 'BBB').catch(GETERR), 'id,type,name').toEqual(
                '@name (BBB) is not same as (AAA)!',
            ); // change to 'BBB' w/o changing model.
            expect2(await $unique.updateLookup({ ...aaa, name: 'BBB' }, 'BBB').catch(GETERR), 'id,type,name').toEqual({
                id: 'aaa',
                type: 'test',
                name: 'BBB',
            }); // change to 'BBB' w/o changing model.
            ...
            expect2(await storage.read($unique.asLookupId('BBB')).catch(GETERR), 'id,type,stereo,meta').toEqual({
                id: '#name/BBB',
                type: 'test',
                stereo: '#',
                meta: 'aaa',
            ...
            expect2(await $unique.updateLookup({ ...bbb }, 'BBB').catch(GETERR), 'id,type,name').toEqual(
