...
import { $engine, _log, _inf, _err, $U, $_, doReportError } from '../engine/';
 ...
 */
export interface CommonS3Service extends CoreServices {
    name(): string;
    hello(): string;
 ...
 */
export class S3Service extends S3CoreService implements CommonS3Service {
    /**
     * environ name config.
     */
    ...
    public constructor(type?: string, idName?: string) {
        super();
        this.bucket = $U.env(S3Service.ENV_S3_NAME, S3Service.DEF_S3_BUCKET);
        this.type = type || 'test';
...
export class DummyS3Service<T extends S3Model> extends S3CoreService implements CommonS3Service {
