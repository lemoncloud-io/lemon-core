...
import { FunctionHandler, AZWEBHandler, AZContext, FunctionSubHandler, AZWEBEvent } from './function-handler';
 ...
 */
export interface AZCoreWEBController {
    hello(): string;
    type(): string;
    decode: NextDecoder;
}
 ...
 * @param $ctx      context
 */
export const promised = async (event: AZWEBEvent, $ctx: NextContext): Promise<ProxyChain> => {
 ...
 */
export const mxNextFailure = (event: AZWEBEvent, $ctx: NextContext) => (e: any) => {
 ...
 */
export class FunctionWEBHandler extends FunctionSubHandler<AZWEBHandler> {
    //! shared config.
    public static REPORT_ERROR: boolean = FunctionHandler.REPORT_ERROR;

    //! handlers map.
    private _handlers: { [key: string]: NextDecoder | AZCoreWEBController } = {};

    /**
     * default constructor w/ registering self.
     */
    public constructor(az_function: FunctionHandler, register?: boolean) {
        super(az_function, register ? 'web' : undefined);
     ...
     */
    public addController(controller: AZCoreWEBController) {
    ...
    public getHandlerDecoders(): { [key: string]: NextDecoder } {
        ...
        return Object.entries(this._handlers).reduce<{ [key: string]: NextDecoder }>((M, [key, val]) => {
            ...
            else M[key] = (m: any, i: any, c: any) => (val as AZCoreWEBController).decode(m, i, c);
            return M;
        }, {});
    }
     ...
     * Default WEB Handler.
     */
    public handle: AZWEBHandler = async (event, $ctx) => {
     ...
     */
    public async handleProtocol<TResult = any>(param: ProtocolParam, event?: any): Promise<TResult> {
        ...
        // const decoder: NextDecoder | AZCoreWEBController = this._handlers[TYPE];
        const next: NextHandler<any, TResult, any> = ((decoder: any) => {
            ...
            if (typeof decoder == 'function') return (decoder as NextDecoder)(MODE, ID, CMD, PATH);
                ...
                const func = (decoder as AZCoreWEBController).decode(MODE, ID, CMD, PATH);
     ...
     */
    public async packContext(event: any, orgContext?: AZContext): Promise<NextContext> {
        _log(NS, `packContext(${event ? '' : 'null'})..`);
        if (!event) return null;
        //! prepare chain object.
        const reqContext: any = event?.requestContext;
...
export interface HttpHeaderTool {
     ...
     */
    prepareContext($org: NextContext, reqContext?: any): Promise<NextContext>;
}
...
export class MyHttpHeaderTool implements HttpHeaderTool {
    protected handler: FunctionWEBHandler;
     ...
     * @param headers
     */
    public constructor(handler: FunctionWEBHandler, headers: HttpHeaderSet) {
