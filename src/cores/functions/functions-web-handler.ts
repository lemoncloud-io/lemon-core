...
import { FunctionHandler, FunctionSubHandler } from './functions-handler';
import { loadJsonSync } from '../../tools/shared';
import { GETERR } from '../../common/test-helper';
import { HEADER_PROTOCOL_CONTEXT } from '../protocol/protocol-service';
import { KeyVaultService, fromBase64 } from '../azure/azure-keyvault-service';
import $protocol from '../protocol/';

const NS = $U.NS('FWEB', 'yellow'); // NAMESPACE TO BE PRINTED.
 ...
 * @param $ctx      context
 */
export const promised = async (ctx: any, req: any): Promise<any> => {
 ...
 */
export const mxNextFailure = (ctx: any, req: any) => (e: any) => {
 ...
 */
export class FunctionWEBHandler extends FunctionSubHandler<any> {
    //! shared config.
     ...
     */
    public constructor(functions: FunctionHandler, register?: boolean) {
        super(functions, register ? 'web' : undefined);
        // _log(NS, `FunctionWEBHandler()..`);
    }
     ...
     * Default WEB Handler.
     */
    public handle: any = async (ctx: any, req: any) => {
        //! inspect API parameters.
        _log(NS, `handle()....`);

        const isInvocationId = ctx.invocationId ? true : false;
        const url = isInvocationId ? req.url : ctx.path;
    ...
    public async handleProtocol<TResult = any>(param: ProtocolParam, ctx: any): Promise<TResult> {
        ...
        if (!next || typeof next != 'function') {
            _err(NS, `! WARN ! MISSING NEXT-HANDLER. event=`, $U.json(ctx));
     ...
     */
    public async packContext(event: any, orgContext?: any): Promise<NextContext> {
...
export class MyHttpHeaderTool implements HttpHeaderTool {
    ...
    protected findKMSService(keyId: string): any {
        ...
        // return aws?.kms;
        const kms = new KeyVaultService(keyId);
        return kms;
    }
