...
/*
 * class: `MocksAWSSQSService`
 * - use <mock>.json file in `./data/mocks/` instead of real AWS SQS request.
 */
export class MocksAWSSQSService extends SQSClient {
        ...
        switch (commandName) {
            case 'SendMessageCommand':
                return {
                    ...
                    MD5OfMessageAttributes: 'mock-md5-attrs',
                };
            case 'ReceiveMessageCommand':
                return {
                    Messages: [
                        {
                            MessageId: `mock-message-id-${Date.now()}`,
                            ReceiptHandle: `mock-receipt-handle-${Date.now()}`,
                            Body: '{"mock": "message", "timestamp": "' + new Date().toISOString() + '"}',
                            MD5OfBody: 'mock-md5-hash',
                            Attributes: {
