...
describe('DummyController', () => {
    ...
    it('should pass basic CRUD w/ dummy', async done => {
        ...
        expect2(await controller.do_list('', { limit: 0 }).catch(GETERR)).toEqual({
            limit: 0,
            list: [],
            page: 1,
            total: 2,
        ...
        expect2(await controller.do_post('A0', null, { type: '' }).catch(GETERR), 'id,type').toEqual({
            id: 'A0',
            type: null,
    ...
    it('should pass basic CRUD w/ dummy-controller', async done => {
        ...
        expect2(await request(app).get(`/${type}?limit=0`), 'status,body').toEqual({
            status: 200,
            body: { limit: 0, list: [], page: 1, total: 2 },
        });
        expect2(await request(app).get(`/${type}/A0`), 'status,body').toEqual({
            status: 200,
            body: { id: 'A0', type: 'user', name: 'lemon' },
        });

        expect2(await request(app).put(`/${type}/A0`).send({ age: 1 }), 'status,body').toEqual({
            status: 200,
            body: { id: 'A0', age: 1, type: 'user', name: 'lemon' },
        });
        expect2(await request(app).get(`/${type}/A0`), 'status,body').toEqual({
            status: 200,
            body: { id: 'A0', age: 1, type: 'user', name: 'lemon' },
        ...
        expect2(await request(app).post(`/${type}/A0`).send({ name: '' }), 'status,body').toEqual({
            status: 200,
            body: { id: 'A0', name: null },
        }); // empty string will be saved as null
        expect2(await request(app).get(`/${type}/A0`), 'status,body').toEqual({
            status: 200,
            body: { id: 'A0', name: null },
        ...
        expect2(await request(app).get(`/${type}/A0`), 'status,text').toEqual({
            status: 404,
            text: '404 NOT FOUND - id:A0',
        });
    });
