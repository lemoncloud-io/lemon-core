...
import * as $core from 'lemon-core';
const _T = $core.$T;
...
export function parsePaginateParam(
    ...
    const MAX_LIMIT = $T.N($core.$U.env('MAX_LIMIT', '2000'));
...
export const $T = {
    ...
    BN: <T extends number = BoolFlag>(v: any, def?: 0 | 1): T => (v === undefined ? (def as T) : (_T.BN(v, def) as T)),
    /** convert to `number` */
